...
class KnowledgeEncoding(nn.Module):
    ...
    def forward(self, img, ques_embed, v_relations, hist_embed, text_rel, batch_size, num_rounds):
        ...
        # img.shape: (b, n_objects, img_feat_dim) -> (b*num_rounds, n_objects, img_feat_dim)
        ...
        # change to shape (batch_size*n_rounds, n_objects, n_objects, new_v_relations_dims)
        v_relations = v_relations.view(
            batch_size,n_objects, n_objects, new_v_relations_dims).repeat(
                1,1,num_rounds,1).view(
                    -1,n_objects,n_objects,new_v_relations_dims)


        ques_embed_v = self.w_v_1(ques_embed)
        # ques_embed_v.shape: (b*num_rounds, 512) -> (b*num_rounds, 36, 36, 512)
        ques_embed_v = ques_embed_v.view(
            batch_size*num_rounds, 1, 1, self.config["lstm_hidden_size"]).repeat(
            1,n_objects,n_objects,1)
        ...
        # Query-Guided Graph Convolution
        img = img.repeat(1,1,n_objects,1).view(int(batch_size*num_rounds),n_objects,n_objects,-1)
        # img.shape = (b*num_rounds, n_objects, n_objects, 2048)
        ...
        # text_rel.shape: (b*num_rounds, num_rounds, num_rounds, 2*512) -> (b*num_rounds, num_rounds, num_rounds, 512)
        ...
        # ques_embed_t.shape: (b*num_rounds, 512) -> (b*num_rounds, n_rounds, n_rounds, 512)
        ques_embed_t = self.w_t_1(ques_embed)
        ques_embed_t = ques_embed_t.view(
            batch_size*num_rounds, 1, 1, 512).repeat(
                1,num_rounds,num_rounds,1)
        ...
        # Query-Guided Graph Convolution
        # hist_embed.shape = (b, num_rounds, num_rounds, 512)
        hist_embed = hist_embed.view(
            batch_size*num_rounds, -1, 1, self.config["lstm_hidden_size"]).repeat(
            1,1,num_rounds,1)
        # hist_embed.shape = (b*num_rounds, num_rounds, num_rounds, 512)
        text_rel_cat = torch.cat((hist_embed, t_relations), -1)
