...
class KnowledgeStorage(nn.Module):
    def __init__(self, config):
        ...
        self.w_v2t_num_rounds = nn.Linear(config["lstm_hidden_size"] + config["img_feature_size"], config["lstm_hidden_size"])
    ...
    def forward(self, v_nodes, t_nodes, ques_embed, batch_size, num_rounds):
        ...
        # Query-Guided Cross Graph Convolution
        t_nodes_t2v = t_nodes.unsqueeze(1).repeat(1,n_objects, 1, 1) # shape: (b*num_rounds, n_objects, num_rounds, 512)
        ...
        # convert question from shape (b*num_rounds, 512) to (b*num_rounds, num_rounds, n_objects, 512)
        ...
        # ques_embed_v2t.shape = (b*num_rounds, num_rounds, n_objects, 512)
        ...
        # Query-Guided Cross Graph Convolution
        v_nodes_v2t = v_nodes.unsqueeze(1).repeat(1,num_rounds, 1, 1)# shape: (b*num_rounds, num_rounds, n_objects, 2048)
        ...
        # Global Knowledge Storage
        q_t2v = ques_embed.unsqueeze(1).repeat(1, n_objects, 1) # shape: (b*num_rounds, n_objects, 512)
        ...
        v_9 = self.w_t2v_9(updated_v_nodes) # shape: (b*num_rounds, n_objects, 512)
        m_v = torch.softmax(self.w_t2v_a(q_t2v * v_9),-2)
        I_c = torch.sum(m_v * updated_v_nodes, -2) # shape: (b*num_rounds, 512)
        ...
        # Global Knowledge Storage
        q_v2t = ques_embed.unsqueeze(1).repeat(1, num_rounds, 1)# shape: (b*num_rounds, num_rounds, 512)
        t_8 = self.w_v2t_8(t_nodes)
        h_t = torch.softmax(self.w_v2t_e(q_v2t * t_8),-2)
        H_o = torch.sum(h_t * t_nodes, -2) # shape: (b*num_rounds, 512)

        t_9 = self.w_v2t_9(updated_t_nodes) # shape: (b*num_rounds, num_rounds, 512)
        m_t = torch.softmax(self.w_v2t_a(q_v2t * t_9),-2)
        H_c = torch.sum(m_t * updated_t_nodes, -2) # shape: (b*num_rounds, 2048)
    ...
    def constructCrossGraphEdges(self, center_nodes, cross_nodes, n_cross, n_center):
        ...
        # mask that indicates where center nodes are empty
        mask1 = center_nodes.abs().sum(dim=-1).bool()
        # Each cross node will be connected to every graph's center node
        cross_nodes = cross_nodes.unsqueeze(1).repeat(1,n_center,1,1)
        # mask that indicates where cross nodes are empty
